...
import com.wteam.ug.weixin.WeixinUtil;
...
//			新方法
            point.setAddress(getAddress(point));
			return point;
		}
		return null;
	}
    ...
    public static String getAddress(Point point) {
        ...
        int pois = 0;
        String coorType = "bd09ll";
        url += "ak=" + ak + "&callback=renderReverse&location=" + point.getLat() + ","
                + point.getLng() + "&output=" + outPut + "&pois=" + pois + "&coordtype="
                + coorType;
        JSONObject object = WeixinUtil.httpRequest(url, "GET", null);
        if (object != null) {
            JSONObject result = object.getJSONObject("result");
     ...
     * @return
     */
    public static String staticMap(Point center, int width,
                                   int height) {
        String url = "http://api.map.baidu.com/staticimage/v2?";
        url += "ak=" + ak + "&width=" + width + "&height=" + height
                + "&center=" + center.getLng() + "," + center.getLat() + "&scale=2&zoom=15";
        url += "&markers=" + center.getLng() + "," + center.getLat();
        return url;
    }
...
//		System.out.println(staticMap(origin, target, 200, 200));
		...
		Point point = geocoder("广东省湛江市麻章区广东海洋大学");
//        Point point1 = antiGeocoder(point.getLat(), point.getLng());
//        System.out.println(point1.getAddress());
//        System.out.println(staticMap(point,200,200));

        Point point1 = geocoder("广州市天河区");
        Point point2 = geocoder("佛山市南海区");
        List<Point> pointList = new ArrayList<>();
        pointList.add(point1);
        pointList.add(point2);
        List<Distance> distanceList = getDistance(point, pointList);
        for (Distance distance : distanceList) {
            System.out.println(distance.getDistanceNum());
            System.out.println(distance.getDistanceText());
            System.out.println(distance.getDurationNum());
            System.out.println(distance.getDurationText());
        }
    }
}
