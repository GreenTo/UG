...
public class payUtils {
    ...
    public void test_trade_pay(AlipayTradeService service) {
        ...
        String subject = "UG维修平台订单消费";

        // (必填) 订单总金额，单位为元，不能超过1亿元
        // 如果同时传入了【打折金额】,【不可打折金额】,【订单总金额】三者,则必须满足如下条件:【订单总金额】=【打折金额】+【不可打折金额】
        String totalAmount = "127.00";

        // (必填) 付款条码，用户支付宝钱包手机app点击“付款”产生的付款条码
        String authCode = "2839604076624396089"; // 条码示例，286648048691290423
    ...
    public static AlipayF2FQueryResult trade_query(String outTradeNo){
        // (必填) 商户订单号，通过此商户订单号查询当面付的交易状态
//        String outTradeNo = "tradepay14817938139942440181";
    ...
    public void test_trade_precreate() {
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String path = "D:/IDEA/projects/ug/src/main/resources/static";
                String filePath = String.format("/image/code/qr-%s.png",
                        response.getOutTradeNo());
                //支付宝调用guava生成二维码
                ZxingUtils.getQRCodeImge(response.getQrCode(), 128, path + filePath);
    ...
    public static String trade_precreate(double money,String description) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = "tradeprecreate" + System.currentTimeMillis()
                + (long) (Math.random() * 10000000L);
